...
func try_snap(point_drag: Node):
	...
	# Enforce accepted dragger rule
	if not allow_any_dragger and point_drag != get_node_or_null(accepted_dragger):
		_play_sound(wrong_snap_sound)
		return

	# Check if inside shape
	var local_mouse = to_local(point_drag.global_position)
	var shape = get_node_or_null("CollisionShape2D") or get_node_or_null("CollisionPolygon2D")
	if not shape or not shape.get_shape().contains_point(local_mouse):
		return
	...
	# Connect drag start to detect unsnap
